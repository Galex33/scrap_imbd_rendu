...
def annee_genre(annee):
  ...
  df_dyn1 = df_dyn.drop(['annee'], axis=1).reset_index()

  list_genre=df_dyn1['genre_final'].to_list()
  list_recette=df_dyn1['recette_final'].to_list()
...
def category_genre():
  ...
  df_genre = new_df.groupby('genre_final').sum('recette_final').sort_values(by= 'recette_final', ascending=False).reset_index()

  height = df_genre['recette_final']
  bars = df_genre['genre_final']
  ...
  # Create title
  plt.title('Classement des genres ayant rapportés les plus grosses recettes toutes années comprises')
