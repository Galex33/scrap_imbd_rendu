...
def recette_year():
  ...
  recette_by_years = sns.lineplot(x="annee", y="recette", data=df_film)
...
def top_recent():
  ...
  df_movies_by_y = df_film.groupby('annee').count()

  df3 = df_movies_by_y.rename_axis('annee').reset_index()

  nb_years = sns.lineplot(x="annee", y="titre", data=df3)
